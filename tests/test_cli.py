...
def test_main_success(mock_subprocess_run, temp_dir):
    with patch(
        "argparse._sys.argv", ["package_orchestrator", "--registry", "test-registry", "--service-name", "test-service"]
    ):
        with patch("package_orchestrator.builder.build_image") as mock_build_image:
            ...
            with patch("builtins.print") as mock_print:
                ...
                mock_print.assert_any_call(
                    "Packaging repository with config: Config(package_registry_url='test-registry', service_name='test-service')"
                )
    ...
    with patch("argparse._sys.argv", ["package_orchestrator", "--registry", "test-registry"]):
        with patch("package_orchestrator.builder.build_image") as mock_build_image:
            ...
            with patch("builtins.print") as mock_print:
                main()
                mock_print.assert_any_call(
                    "Packaging repository with config: Config(package_registry_url='test-registry', service_name='my-service')"
                )
